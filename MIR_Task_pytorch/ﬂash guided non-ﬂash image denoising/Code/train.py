...
from CUdata_npy_denoise25 import cudataset
...
class Trainer:
    def __init__(self):
        ...
        if os.path.exists('denoise_model_25/latest.pth'):
            print('===> Loading pre-trained model...')
            state = torch.load('denoise_model_25/latest.pth')
    ...
    def train(self):
        ...
        for ep in range(1, self.epoch + 1):
            ...
            if not os.path.exists("denoise_model_25/"):
                os.makedirs("denoise_model_25/")
            torch.save(state, os.path.join('denoise_model_25/', 'latest.pth'))
            torch.save(state, os.path.join('denoise_model_25/', str(ep) + '.pth'))
            matplotlib.use('Agg')
            plot_loss_list = self.train_loss
            fig1 = plt.figure()
            plt.plot(plot_loss_list)
            plt.savefig('train_loss_curve_cunet_denoise25.png')
            plt.close('all')
