...
class cudataset(data.Dataset):
    def __init__(self):
        ...
        self.depth = np.load('trainset/multi_exposure/data_over.npy')#(batch,height,width,c)
        self.depth = np.transpose(self.depth, (0, 3, 1, 2))
        self.depth_t = torch.from_numpy(self.depth)

        self.gt = np.load('trainset/multi_exposure/label.npy')  # (batch,height,width,c)
        self.gt = np.transpose(self.gt, (0, 3, 1, 2))
        self.gt_t = torch.from_numpy(self.gt)

        self.rgb = np.load('trainset/multi_exposure/data_under.npy')  # (batch,height,width,c)
